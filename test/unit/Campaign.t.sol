...
contract CampaignTest is CampaignFixtures {
    ...
    function test_stateTransition_ReachGoal_StaysActive() public {
        contributeAndReachGoal(campaign);
        assertEq(uint256(campaign.state()), uint256(CampaignState.Active));
    }

    function test_stateTransition_AfterDeadline_StillAcceptsContributions() public {
        // Warp past deadline without reaching goal
        vm.warp(block.timestamp + 31 days); // Past the default 30-day duration

        // In the new model, campaigns don't automatically fail after deadline
        // They stay active until explicitly cancelled
    ...
    function test_contribute_AfterDeadline_StillWorks() public {
        vm.warp(block.timestamp + 31 days); // Past the default 30-day duration

        // In new model, contributions still work after deadline
        vm.prank(CONTRIBUTOR_1);
        campaign.contribute{value: 1 ether}();
    ...
    function test_withdrawFunds_NoFunds_Reverts() public {
        vm.prank(CREATOR);
