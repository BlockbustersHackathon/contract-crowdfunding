...
contract PricingCurveTest is CampaignFixtures {
    ...
    function test_calculateTokensForContribution_BasicCalculation() public {
        ...
        (uint256 tokenAmount, uint256 tier) = pricingCurve.calculateTokensForContribution(1 ether, 0, 10 ether, tiers);
    ...
    function test_calculateTokensForContribution_TierBonusApplied() public {
        ...
        (uint256 tokens1,) = pricingCurve.calculateTokensForContribution(0.5 ether, 0, 10 ether, tiers);

        // Large contribution (tier 2 - 10% bonus)
        (uint256 tokens2,) = pricingCurve.calculateTokensForContribution(5 ether, 0, 10 ether, tiers);
    ...
    function test_getDiscountedPrice_EarlyBird_AppliesDiscount() public {
        ...
        uint256 discountedPrice = pricingCurve.getDiscountedPrice(basePrice, startTime + 1 days, startTime);
    ...
    function test_getDiscountedPrice_AfterEarlyBird_NoDiscount() public {
        ...
        uint256 price = pricingCurve.getDiscountedPrice(basePrice, startTime + 8 days, startTime);
    ...
    function test_calculateTokensWithEarlyBird_AppliesBothBonuses() public {
        ...
        (uint256 tokensEarly,) =
            pricingCurve.calculateTokensWithEarlyBird(1 ether, 0, 10 ether, startTime + 1 days, startTime, tiers);

        // Calculate without early bird bonus
        (uint256 tokensLater,) =
            pricingCurve.calculateTokensWithEarlyBird(1 ether, 0, 10 ether, startTime + 8 days, startTime, tiers);
    ...
    function test_calculateAveragePrice_ReturnsReasonableValue() public {
        uint256 averagePrice = pricingCurve.calculateAveragePrice(2 ether, 5 ether, 10 ether);
