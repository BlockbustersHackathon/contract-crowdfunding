...
contract AccessControlTest is CampaignFixtures {
    ...
    function test_treasury_CampaignFunctions_RestrictAccess() public {
        // Note: This test assumes treasury can validate campaign addresses
        // In the current implementation, we need to test differently
        
        // Non-campaign cannot deposit directly
        vm.prank(CONTRIBUTOR_1);
    ...
    function test_emergency_TreasuryWithdraw_OwnerOnly() public {
        // Put some funds in the treasury first
        vm.deal(address(treasury), 5 ether);
        
        // Non-owner cannot emergency withdraw
        vm.prank(CREATOR);
        vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, CREATOR));
        treasury.emergencyWithdraw(campaign.campaignId(), CREATOR);
        
        // Owner can emergency withdraw
        vm.prank(ADMIN);
        treasury.emergencyWithdraw(campaign.campaignId(), ADMIN);
    }
    
    // ============ State-Based Access Control ============
    
    function test_stateBased_WithdrawOnlyInSuccessful() public {
        // Cannot withdraw in Active state
        vm.prank(CREATOR);
