...
contract TokenFactory {
    event TokenCreated(address indexed tokenAddress, address indexed campaignAddress, string name, string symbol);

    function createToken(string memory name, string memory symbol, address campaignAddress, address owner)
        external
        returns (address)
    {
        ...
        CampaignToken token = new CampaignToken(name, symbol, campaignAddress, owner);
