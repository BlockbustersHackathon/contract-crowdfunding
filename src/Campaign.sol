...
contract Campaign is ICampaign, ICampaignEvents, ReentrancyGuard, Ownable {
    ...
    function contribute() external payable nonReentrant onlyActiveState campaignNotExpired {
        ...
        uint256 timeRemaining = campaignData.deadline > block.timestamp 
            ? campaignData.deadline - block.timestamp 
            : 0;
    ...
    function withdrawFunds() external nonReentrant onlyCreator {
        require(
            campaignData.state == CampaignState.Succeeded,
            "Campaign: Campaign must be successful"
        );
        ...
        payable(campaignData.creator).transfer(amount);
    ...
    function refund() external nonReentrant {
        ...
        require(
            ...
            campaignData.state == CampaignState.RefundsAvailable,
    ...
    function createLiquidityPool() external nonReentrant onlyCreator {
        ...
        (uint256 tokenAmount, uint256 ethAmount, uint256 liquidity) = dexIntegrator.addLiquidity{value: ethForLiquidity}(
        ...
        if (remainingETH > 0) {
            payable(campaignData.creator).transfer(remainingETH);
        }
    ...
    function extendDeadline(uint256 newDeadline) external onlyCreator onlyActiveState {
        ...
        require(
            newDeadline <= campaignData.deadline + EXTENSION_LIMIT,
            "Campaign: Extension exceeds limit"
        );
